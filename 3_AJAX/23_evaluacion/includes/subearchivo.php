...
if (!((strpos($tipo_archivo, "gif") || strpos($tipo_archivo, "jpeg")) && ($tamano_archivo < 100000))) {
    ...
    $rutaReferenciaArchivo = $image_dir . "/" . $nombre_archivo;
    if (move_uploaded_file($_FILES['userfile']['tmp_name'], $rutaArchivo)){
        ...
        if($idPropiedad != "0"){
            $selectSql = "SELECT MAX(orden) AS Orden FROM `imagenes` WHERE id_propiedad = " . $idPropiedad;
        }
        else
        {
            $selectSql = "SELECT MAX(orden) AS Orden FROM 'imagenes' WHERE id_propiedad IS NULL";
        }
        ...
        $insertSql.= " VALUES (" . $idPropiedad . ", '" . $rutaReferenciaArchivo . "'," ;
        $insertSql.= $orden . ")";
